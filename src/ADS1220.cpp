...
Protocentral_ADS1220::Protocentral_ADS1220(SPIClass *spi) 								// Constructors
{
     SPI1 = spi;
}
...
void Protocentral_ADS1220::begin(uint8_t cs_pin, uint8_t drdy_pin)
{
    ...
    SPI1->setFrequency(100000); // Zum testen, ob die SPI-Kommunikation stabiler ist
...
bool Protocentral_ADS1220::IsDataReady() 
{
    return (digitalRead(m_drdy_pin) == LOW);
}

/*
Konfigurieren Sie das Gerät für Single-Ended-Messungen, indem Sie entweder einen der Analogeingänge anschließen
an AVSS extern oder über die interne AVSS-Verbindung des Multiplexers (MUX[3:0]-Einstellungen 1000 bis
1011). Beim Konfigurieren des internen Multiplexers für Einstellungen mit AINN = AVSS (MUX[3:0] = 1000 bis
1011) wird der PGA automatisch umgangen und deaktiviert, unabhängig von der PGA_BYPASS-Einstellung und der Verstärkung ist
auf 1, 2 und 4 begrenzt. Falls die Verstärkung auf mehr als 4 eingestellt ist, begrenzt das Gerät die Verstärkung auf 4.
*/
...
uint8_t Protocentral_ADS1220::get_mux_channel() 
{
 	uint8_t config = readRegister(CONFIG_REG0_ADDRESS);
	return ( config & REG_CONFIG0_MUX_MASK);
}
...
int32_t Protocentral_ADS1220::Read_WaitForData()
{
    ...
    if((digitalRead(m_drdy_pin)) == LOW)             //        Wait for DRDY to transition low
    {
        ...
        for (int i = 0; i < 3; i++)
        {
          SPI_Buff[i] = SPI1->transfer(SPI_MASTER_DUMMY);
        }
        delayMicroseconds(100);
